...
\abstract{Negation is a fundamental element of language and logical systems, but processing negative sentences can be challenging. Previous work suggests that a supportive context can mitigate the processing costs of negation.  We propose a pragmatic explanation for these past findings: Speakers only produce negative sentences when they are both relevant and informative, and listeners are fastest to respond to sentences that they expect speakers to produce.  We find that speakers are most likely to produce negative sentences that negate a salient feature of the context (relevance) and effectively identify the referent of their utterance (informativeness), and that participants' reaction times reflect expectations about how a rational speaker would describe the same stimuli.  Our data suggest that representing negation may be less difficult than previously supposed, and that negative sentences are only difficult when they are unlikely to be produced.}  
...
Our findings here suggest that a large part of the processing difficulties of negative sentences arise from the relative pragmatic felicity of negation in context. They do not rule out the possibility that there is some cost to processing an additional logical element, but this processing cost would have to be small with respect to the magnitude of the effects we observed. When logical words are used in a communicative context, we have no difficulty understanding them.  \blue{is this aspirational enough?}
